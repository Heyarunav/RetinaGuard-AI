function RetinaGuard_App()
    % =====================================================================
    % RETINA GUARD AI - ABDM TELE-OPHTHALMOLOGY & GEO-AWARE TRIAGE
    % Dual Mode (Doctor/ASHA) + Dynamic State Language Sync + Native File Dialog
    % =====================================================================
    ...
    abhaGrid = uigridlayout(col1Grid, [1, 3]);
    abhaGrid.ColumnWidth = {110, '1x', 80};
    abhaGrid.Padding = [0 0 0 0];
    lblAbhaPrompt = uilabel(abhaGrid, 'Text', 'Patient ABHA ID:', 'FontSize', 11, 'FontWeight', 'bold', 'FontColor', [1 1 1]);
    txtAbha = uieditfield(abhaGrid, 'text', 'Value', '91-4821-3904-7120', 'FontSize', 11, 'FontWeight', 'bold');
    
    btnAbhaHistory = uibutton(abhaGrid, 'push', ...
        'Text', '📋 Intake', ...
        'FontSize', 10, 'FontWeight', 'bold', ...
        'BackgroundColor', [0.15 0.35 0.6], 'FontColor', [1 1 1], ...
        'ButtonPushedFcn', @(btn, event) showPatientIntakeModal());
    ...
    % =====================================================================
    % EXPORT REFERRAL SLIP (OFFLINE QUEUE + MULTIMODAL OCULOMICS)
    % =====================================================================
    ...
    function exportReport()
        ...
        % 1. Multimodal Oculomics Risk Stratification (Retina + HbA1c + BP)
        oculomicsWarning = 'Standard Microvascular Monitoring';
        hba1cVal = 7.0;
        bpVal = 'Normal';
        if isfield(appData, 'hasVitals') && appData.hasVitals
            hba1cVal = appData.patientHbA1c;
            bpVal = appData.patientBP;
            if hba1cVal >= 8.0 && ~strcmp(stage, 'No Diabetic Retinopathy')
                oculomicsWarning = 'HIGH RISK: Concurrent Diabetic Nephropathy / Capillary Micro-Stroke Suspected (HbA1c >= 8.0%)';
            end
        end

        % 2. Build HL7 FHIR v4.0.1 DiagnosticReport Payload
        cleanAbha = regexprep(txtAbha.Value, '[^0-9]', '');
        fhirPayload = struct(...
            'resourceType', 'DiagnosticReport', ...
            'id', cleanAbha, ...
            'status', 'final', ...
            'category', 'Ophthalmology-TeleTriage', ...
            'code', struct('coding', struct('system', 'http://snomed.info/sct', 'code', '422034002', 'display', 'Diabetic Retinopathy Staging')), ...
            'subject', struct('reference', ['Patient/' txtAbha.Value]), ...
            'conclusion', stage, ...
            'confidence', sprintf('%.1f%%', appData.confidence * 100), ...
            'actionDirective', badgeRisk.Text, ...
            'systemicOculomicsRisk', oculomicsWarning, ...
            'clinicalVitals', struct('HbA1c', sprintf('%.1f%%', hba1cVal), 'BloodPressure', bpVal), ...
            'dispatchMode', ternary(appData.isOnline, 'Live ABDM Cloud Gateway', 'Encrypted Local Offline Queue'), ...
            'regionalNotification', regionalMsg);

        % 3. Save JSON Locally (Disk Cache)
        jsonFileName = ternary(appData.isOnline, 'ABDM_FHIR_DiagnosticReport.json', 'OFFLINE_QUEUE_DiagnosticReport.json');
        try
            fid = fopen(fullfile(pwd, jsonFileName), 'w');
            if fid ~= -1
                try
                    fprintf(fid, '%s', jsonencode(fhirPayload, 'PrettyPrint', true));
                catch
                    fprintf(fid, '%s', jsonencode(fhirPayload));
                end
                fclose(fid);
            end
        catch err
            disp(['File write warning: ' err.message]);
        end

        % 4. Present Modal Summary
        syncStatusText = ternary(appData.isOnline, ...
            '● DISPATCH: Live sync to e-Sanjeevani Specialty Gateway confirmed.', ...
            '● STORE & FORWARD: Device offline. Record encrypted & stored in local queue.');
        ...
        summaryText = sprintf([...
            ...
            'Multimodal Vitals   : HbA1c: %.1f%% | BP: %s\n', ...
            'Oculomics Triage    : %s\n', ...
            'Frontline Action    : %s\n\n', ...
            '-------------------------------------------------------\n', ...
            ...
            '-------------------------------------------------------\n', ...
            '%s\n', ...
            'Local Record Cached : %s'], ...
            txtAbha.Value, region, appData.currentMode, ternary(appData.isOnline, 'ONLINE', 'OFFLINE'), ...
            stage, appData.confidence * 100, hba1cVal, bpVal, oculomicsWarning, ...
            badgeRisk.Text, engMsg, hindiMsg, region, regionalMsg, syncStatusText, jsonFileName);

        uialert(fig, summaryText, 'ABDM Referral Processed', 'Icon', 'success');
    end
    ...
    % =====================================================================
    % PATIENT LONGITUDINAL RECORD & MANUAL INTAKE MODAL
    % =====================================================================
    function showPatientIntakeModal()
        enteredAbha = strtrim(txtAbha.Value);

        % Check if ABHA matches mock registry record
        if strcmp(enteredAbha, '91-4821-3904-7120')
            % Store verified clinical vitals for multimodal systemic triage
            appData.patientName = 'Ramesh Gogoi (Age: 58)';
            appData.patientHbA1c = 8.4;
            appData.patientBP = '142/90';
            appData.hasVitals = true;

            recordMsg = sprintf([...
                'ABDM HEALTH REPOSITORY RECORD FOUND\n', ...
                '--------------------------------------------------\n', ...
                'Patient Name    : Ramesh Gogoi (Age: 58, Male)\n', ...
                'ABHA ID         : 91-4821-3904-7120\n', ...
                'Linked PHC      : Rangia Primary Health Centre\n\n', ...
                'LONGITUDINAL CLINICAL VITALS (Synchronized):\n', ...
                '• Diabetes History    : Type-2 (Diagnosed 7 yrs ago)\n', ...
                '• Last Recorded HbA1c : 8.4%% (Uncontrolled)\n', ...
                '• Blood Pressure      : 142/90 mmHg (Stage 1 HTN)\n', ...
                '--------------------------------------------------\n', ...
                'STATUS: Linked to current session. Systemic Oculomics enabled.']);
            uialert(fig, recordMsg, 'ABDM Patient Profile Found', 'Icon', 'info');
        else
            manualPrompt = {
                'Enter Patient Full Name & Age:', ...
                'Duration of Diabetes (Years):', ...
                'Recent HbA1c Level (% e.g., 8.1):', ...
                'Current Blood Pressure (e.g., 135/85):'
            };
            dlgTitle = 'Frontline Manual Patient Intake (No ABHA Linked)';
            defaultVal = {'Anonymous Patient, 52', '4', '8.1', '135/85'};
            manualData = inputdlg(manualPrompt, dlgTitle, [1 45], defaultVal);

            if ~isempty(manualData)
                appData.patientName = manualData{1};
                appData.patientHbA1c = str2double(manualData{3});
                if isnan(appData.patientHbA1c); appData.patientHbA1c = 7.0; end
                appData.patientBP = manualData{4};
                appData.hasVitals = true;

                uialert(fig, sprintf([...
                    'MANUAL CLINICAL BASELINE RECORDED:\n\n', ...
                    '• Patient  : %s\n', ...
                    '• Duration : %s yrs\n', ...
                    '• HbA1c    : %.1f%%\n', ...
                    '• BP       : %s\n\n', ...
                    'Vitals linked for multimodal risk calculation.'], ...
                    appData.patientName, manualData{2}, appData.patientHbA1c, appData.patientBP), ...
                    'Manual Intake Saved', 'Icon', 'success');
            end
        end
    end
end
